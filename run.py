...
from src.config import (
    SVG_INPUT_DIR, GCODE_OUTPUT_DIR, FEED, 
...
def write_gcode(paths, scale: float) -> List[str]:
    ...
    """
    g: List[str] = []
    g += ["G90", "G21", CMD_UP]   # absoluto, mm, altura segura

    # Calcular bounding box para girar 180° respecto al centro
    xmin, xmax, ymin, ymax = get_svg_bbox(paths)
    cx, cy = (xmin + xmax) / 2, (ymin + ymax) / 2

    logger.info("Bounding box: xmin=%.3f, xmax=%.3f, ymin=%.3f, ymax=%.3f", xmin, xmax, ymin, ymax)
    logger.info("Centro de rotación: cx=%.3f, cy=%.3f", cx, cy)
    logger.info("Escala aplicada: %.3f", scale)
    ...
    for idx, p in enumerate(paths):
        ...
        first_point = True
        path_gcode_count = 0  # <-- contador de instrucciones para este path
        for x, y in sample_path(p, STEP_MM):
            x, y = rotate180(x, y)
            x, y = mirror_horizontal(x, y)
            x_mm, y_mm = x * scale, y * scale
            if first_point:
                logger.info("Inicio de trazo %d: X=%.3f, Y=%.3f", idx+1, x_mm, y_mm)
                g += [f"G0 X{x_mm:.3f} Y{y_mm:.3f}", CMD_DOWN, f"G4 P{DWELL_MS / 1000.0:.3f}"]
                path_gcode_count += 3  # <-- suma las instrucciones agregadas arriba
                first_point = False
            g.append(f"G1 X{x_mm:.3f} Y{y_mm:.3f} F{FEED}")
            path_gcode_count += 1  # <-- suma una instrucción por cada punto
        # Al terminar el trazo, levantar lapicera
        g += [CMD_UP, f"G4 P{DWELL_MS / 1000.0:.3f}"]
        path_gcode_count += 2  # <-- suma las instrucciones finales del trazo
        logger.info(
            "Fin de trazo %d, comandos: %s, G4 P%.3f",
            idx+1, CMD_UP, DWELL_MS/1000.0
        )
        logger.debug("Comando M5 (apagar herramienta) emitido tras trazo %d", idx+1)
        logger.info("Instrucciones G-code generadas para path %d: %d", idx+1, path_gcode_count)  # <-- NUEVO

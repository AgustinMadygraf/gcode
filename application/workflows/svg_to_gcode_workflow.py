...
class SvgToGcodeWorkflow:
    ...
    def run(self, selector=None):
        ...
        logger.debug(self.i18n.get("debug_workflow_started"))
        with PerformanceTimer.measure(logger, "SVG to GCODE Workflow"):  # Medición de tiempo
            ...
            if selector and getattr(selector, 'i18n', None) is None:
                logger.debug(self.i18n.get("debug_selector_no_i18n"))
            ...
            logger.debug(self.i18n.get("INFO_PROCESSING_FILE"))
            try:
                paths = svg_loader_factory(svg_file).get_paths()
                logger.debug(self.i18n.get("debug_svg_paths_extracted", count=len(paths) if paths else 0))
            except (OSError, ValueError) as e:
                logger.error(self.i18n.get("error_loading_svg_paths", error=str(e)))
                logger.error(self.i18n.get("error_no_svg"))
                return False
            logger.debug(self.i18n.get("INFO_PROCESSING_DONE"))
            ...
            try:
                ...
                logger.debug(self.i18n.get("debug_bbox_calculated", bbox=str(bbox)))
            ...
            try:
                ...
                logger.debug(self.i18n.get("debug_center_calculated", cx=_cx, cy=cy))
            ...
            if self.config.get_mirror_vertical():
                ...
                logger.debug(self.i18n.get("debug_vertical_mirror_applied"))
            ...
            for i, _ in enumerate(gcode_lines, 1):
                if i % max(1, total_lines // 100) == 0 or i == total_lines:
                    if i % max(1, total_lines // 10) == 0 or i == total_lines:
                        logger.debug(self.i18n.get("debug_gcode_write_progress_simple", current=i, total=total_lines))
            try:
                with gcode_file.open("w", encoding="utf-8") as f:
                    f.write("\n".join(gcode_lines))
                logger.debug(self.i18n.get("INFO_GCODE_WRITTEN", filename=gcode_file_str))

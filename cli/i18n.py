...
MESSAGES = {
    ...
    'zh': {
        'DEBUG_DEV_MODE_ON': "[开发] 开发者模式已激活：DEBUG 日志和扩展堆栈追踪。",
        'MENU_MAIN_TITLE': "主菜单",
        'MENU_OPTION_CONVERT': "1. SVG 转 G-code",
        'MENU_OPTION_OPTIMIZE': "2. 优化已有 G-code 文件",
        'PROMPT_SELECT_OPTION': "请输入选项编号：",
        'INFO_SVG_FILES_FOUND': "找到的 SVG 文件：",
        'OPTION_SVG_FILE': "[{num}] {filename}",
        'OPTION_CANCEL': "[0] 取消",
        'PROMPT_SELECT_SVG_FILE': "请输入 SVG 文件编号：",
        'INFO_SVG_SELECTED': "已选择 SVG 文件：{filename}",
        'INFO_GCODE_OUTPUT': "输出 G-code 文件：{filename}",
        'INFO_PROCESSING_FILE': "正在处理文件……这可能需要几秒钟。",
        'INFO_PROCESSING_DONE': "处理完成",
        'OPTION_TOOL_PEN': "[1] 钢笔（仅轮廓）",
        'OPTION_TOOL_MARKER': "[2] 粗记号笔（填充/轮廓）",
        'PROMPT_SELECT_TOOL': "请选择一个选项：",
        'OPTION_YES': "[y] 是",
        'OPTION_NO': "[n] 否",
        'PROMPT_DOUBLE_PASS': "轮廓双遍？[Y/n]：",
        'INFO_SVG_LOAD': "已加载 SVG：{filename}",
        'INFO_PATHS_EXTRACTED': "提取路径数：{count}",
        'INFO_PATHS_PROCESSED': "处理后有效路径数：{count}",
        'DEBUG_PATHS_ORDER_ORIG': "原始路径顺序：{list}",
        'DEBUG_TOTAL_DIST_ORIG': "原始总距离：{dist} 毫米",
        'DEBUG_PATHS_ORDER_OPT': "优化后路径顺序：{list}",
        'DEBUG_TOTAL_DIST_OPT': "优化后总距离：{dist} 毫米",
        'DEBUG_BOUNDING_BOX': "边界框：xmin={xmin}, xmax={xmax}, ymin={ymin}, ymax={ymax}",
        'DEBUG_SCALE_APPLIED': "应用缩放：{scale}",
        'DEBUG_RELATIVE_MOVES': "已启用相对移动：{enabled}",
        'DEBUG_ARC_COMPRESS': "ArcCompressor 压缩率：{percent}%",
        'DEBUG_LINE_COMPRESS': "LineCompressor 压缩率：{percent}%",
        'DEBUG_GCODE_LINES': "生成的 G-code 行数：{count}",
        'DEBUG_OPTIMIZATION_METRICS': "优化指标：{metrics}",
        'INFO_GCODE_GENERATED': "已生成 G-code，共 {count} 行",
        'DEBUG_COMPRESSION_SUMMARY': "压缩：原始={orig}，压缩后={comp}，比率={ratio}%",
        'INFO_GCODE_WRITTEN': "已写入 G-code 文件：{filename}",
        'INFO_GCODE_SUCCESS': "✔ G-code 生成成功：{filename}",
        'ERROR_INVALID_SELECTION': "无效选择。请选择有效选项。",
        'ERROR_GENERIC': "发生了意外错误。",
        'WARN_INVALID_SELECTION': "无效选择。请重试。",
        'WARN_INVALID_NUMBER': "请输入有效数字。",
        'WARN_YES_NO': "请回答 'y'（是）或 'n'（否）。",
        'INFO_EXIT': "\n正在退出程序。再见！",
        'INFO_EXIT_INTERRUPT': "\n因中断（Ctrl+C）退出程序。",
        ...
        'ARGPARSE_DESCRIPTION': (
            "将 SVG 文件转换为简单绘图仪或 CNC 的 G-code 路径。\n\n"
            "用法示例：\n"
            "  python run.py --input 示例.svg --output 结果.gcode\n"
            "  python run.py --no-interactive -i 示例.svg -o - > 结果.gcode\n"
            "  cat 示例.svg | python run.py --no-interactive -i - -o 结果.gcode\n"
            "  python run.py --no-interactive --optimize -i 输入.gcode -o 优化后.gcode\n\n"
            "使用 -h 或 --help 查看所有选项。"
        ),
        'ARG_NO_INTERACTIVE': "以非交互模式运行",
        'ARG_NO_COLOR': "禁用彩色输出",
        'ARG_LANG': "界面语言（es, en, zh）",
        'ARG_INPUT': "输入 SVG 或 G-code 文件（使用 '-' 表示标准输入）",
        'ARG_OUTPUT': "输出 G-code 文件（使用 '-' 表示标准输出）",
        'ARG_OPTIMIZE': "应用移动优化",
        'ARG_RESCALE': "G-code 文件的缩放因子",
        'ARG_SAVE_CONFIG': "将当前参数保存为用户配置",
        'ARG_CONFIG': "自定义配置文件路径（JSON）",
        'ARG_TOOL': "工具类型：钢笔或记号笔",
        'ARG_DOUBLE_PASS': "钢笔轮廓使用双遍",
        'ARG_NO_DOUBLE_PASS': "禁用轮廓双遍",
        'ARG_DEV': "启用开发者模式（DEBUG 日志，扩展堆栈追踪）",
    }
}
